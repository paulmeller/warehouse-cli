...
Warehouse is a macOS-focused CLI tool that consolidates personal data (iMessages, contacts, photos, Obsidian notes, documents, reminders) into a single SQLite database with FTS5 full-text search. The primary workflow is: `sync` (extract from sources) → `index` (build FTS5 indexes) → `search`/`browse`.
...
- `fts.rs` — Rebuilds all 6 FTS indexes from source tables, manages FTS↔source rowid mappings
