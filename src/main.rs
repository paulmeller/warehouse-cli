...
fn cmd_status(db_path: &str) -> Result<()> {
    ...
    let tables = [
        ...
        ("likes", "twitter_likes"),
        ("monarch_txns", "monarch_transactions"),
        ("pocketsmith_txns", "pocketsmith_transactions"),
    ];
...
fn cmd_doctor() -> Result<()> {
    ...
    // PocketSmith
    let cfg = config::load_config();
    if cfg.pocketsmith.enabled && !cfg.pocketsmith.api_key.is_empty() {
        println!("  PocketSmith: API key configured (OK)");
    } else {
        println!("  PocketSmith: not configured");
    }
    ...
    if db_exists {
        ...
        let tables = [
            ...
            ("bookmarks", "twitter_bookmarks"),
            ("likes", "twitter_likes"),
            ("transactions", "pocketsmith_transactions"),
        ];
        for (label, table) in &tables {
            let count = db::table_count(&conn, table);
            if count > 0 {
