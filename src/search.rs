...
pub fn fts_search(conn: &Connection, query: &str, options: &SearchOptions) -> Result<Vec<SearchResult>> {
    ...
    if types.contains(&"bookmark") {
        results.extend(search_bookmarks_fts(conn, &fts_query, options)?);
    }
    if types.contains(&"like") {
        results.extend(search_likes_fts(conn, &fts_query, options)?);
    }
    if types.contains(&"transaction") {
        results.extend(search_transactions_fts(conn, &fts_query, options)?);
    }
...
fn search_bookmarks_fts(
    ...
    let mut sql = String::from(
        "SELECT
            'bookmark' as type,
            map.tweet_id as id,
            '@' || b.author_handle as title,
            snippet(bookmarks_fts, 0, '**', '**', '...', 32) as snippet,
            ...
            b.retweet_count
        FROM bookmarks_fts
        JOIN bookmarks_fts_map map ON bookmarks_fts.rowid = map.fts_rowid
        JOIN twitter_bookmarks b ON map.tweet_id = b.tweet_id
    ...
    if let Some(ref from) = options.date_from {
        sql.push_str(&format!(" AND b.created_at >= ?{}", params.len() + 1));
        params.push(Box::new(from.clone()));
    }
    if let Some(ref to) = options.date_to {
        sql.push_str(&format!(" AND b.created_at <= ?{}", params.len() + 1));
    ...
    sql.push_str(" ORDER BY bm25(bookmarks_fts) LIMIT 30");
    ...
    match conn.prepare(&sql) {
        Ok(mut stmt) => {
            let rows = stmt.query_map(param_refs.as_slice(), |row| {
                ...
                let favorites: Option<i64> = row.get("favorite_count")?;
                let retweets: Option<i64> = row.get("retweet_count")?;
                let mut metadata = HashMap::new();
                if let Some(an) = author_name {
                    metadata.insert("author_name".into(), serde_json::Value::String(an));
                }
                if let Some(ca) = created_at {
                    metadata.insert("created_at".into(), serde_json::Value::String(ca));
                }
                if let Some(f) = favorites {
                    metadata.insert("favorites".into(), serde_json::json!(f));
                }
                if let Some(rt) = retweets {
                    metadata.insert("retweets".into(), serde_json::json!(rt));
                }
                Ok(SearchResult {
                    result_type: "bookmark".into(),
                    id: row.get("id")?,
                    title: row.get::<_, Option<String>>("title")?.unwrap_or_else(|| "Unknown".into()),
                    snippet: row.get::<_, Option<String>>("snippet")?.unwrap_or_default(),
                    score: score.abs(),
                    metadata,
                })
...
fn search_likes_fts(
    ...
    let mut sql = String::from(
        "SELECT
            'like' as type,
            map.tweet_id as id,
            '@' || l.author_handle as title,
            snippet(likes_fts, 0, '**', '**', '...', 32) as snippet,
            bm25(likes_fts) as score,
            l.author_name,
            l.created_at,
            l.favorite_count,
            l.retweet_count
        FROM likes_fts
        JOIN likes_fts_map map ON likes_fts.rowid = map.fts_rowid
        JOIN twitter_likes l ON map.tweet_id = l.tweet_id
        WHERE likes_fts MATCH ?1",
    );

    let mut params: Vec<Box<dyn rusqlite::types::ToSql>> = vec![Box::new(query.to_string())];

    if let Some(ref from) = options.date_from {
        sql.push_str(&format!(" AND l.created_at >= ?{}", params.len() + 1));
        params.push(Box::new(from.clone()));
    }
    if let Some(ref to) = options.date_to {
        sql.push_str(&format!(" AND l.created_at <= ?{}", params.len() + 1));
    ...
    sql.push_str(" ORDER BY bm25(likes_fts) LIMIT 30");
...
fn search_transactions_fts(
    ...
    // Monarch transactions
    {
        let mut sql = String::from(
            "SELECT
                'transaction' as type,
                map.transaction_id as id,
                'monarch' as source,
                COALESCE(t.merchant_name, 'Unknown Merchant') as title,
                snippet(transactions_fts, 0, '**', '**', '...', 32) as snippet,
            ...
            JOIN transactions_fts_map map ON transactions_fts.rowid = map.fts_rowid
            JOIN monarch_transactions t ON map.transaction_id = t.id
            WHERE transactions_fts MATCH ?1
              AND map.source = 'monarch'",
        );

        let mut params: Vec<Box<dyn rusqlite::types::ToSql>> = vec![Box::new(query.to_string())];

        if let Some(ref from) = options.date_from {
            sql.push_str(&format!(" AND t.date >= ?{}", params.len() + 1));
            params.push(Box::new(from.clone()));
        }
        if let Some(ref to) = options.date_to {
            sql.push_str(&format!(" AND t.date <= ?{}", params.len() + 1));
        ...
        sql.push_str(" ORDER BY bm25(transactions_fts) LIMIT 15");
        ...
        if let Ok(mut stmt) = conn.prepare(&sql) {
            if let Ok(rows) = stmt.query_map(param_refs.as_slice(), |row| {
                ...
                let category: Option<String> = row.get("category_name")?;
                let account: Option<String> = row.get("account_name")?;
                let notes: Option<String> = row.get("notes")?;
                let mut metadata = HashMap::new();
                if let Some(a) = amount {
                    metadata.insert("amount".into(), serde_json::json!(a));
                }
                if let Some(d) = &date {
                    metadata.insert("date".into(), serde_json::Value::String(d.clone()));
                }
                if let Some(c) = &category {
                    metadata.insert("category".into(), serde_json::Value::String(c.clone()));
                }
                if let Some(ac) = account {
                    metadata.insert("account".into(), serde_json::Value::String(ac));
                }
                if let Some(n) = notes {
                    metadata.insert("notes".into(), serde_json::Value::String(n));
                }
                metadata.insert("source".into(), serde_json::Value::String("monarch".into()));
                let amt = amount.unwrap_or(0.0);
                let cat = category.unwrap_or_else(|| "Uncategorized".into());
                Ok(SearchResult {
                    result_type: "transaction".into(),
                    id: row.get("id")?,
                    title: row.get::<_, Option<String>>("title")?.unwrap_or_else(|| "Unknown".into()),
                    snippet: format!("${:.2} - {cat}", amt.abs()),
                    score: score.abs(),
                    metadata,
                })
            }) {
    ...
    // PocketSmith transactions
    {
        let mut sql = String::from(
            "SELECT
                'transaction' as type,
                map.transaction_id as id,
                'pocketsmith' as source,
                COALESCE(t.payee, 'Unknown Payee') as title,
                snippet(transactions_fts, 0, '**', '**', '...', 32) as snippet,
                bm25(transactions_fts) as score,
                t.amount,
                t.date,
                t.category_name,
                t.transaction_account_name as account_name,
                COALESCE(t.note, '') || ' ' || COALESCE(t.memo, '') as notes
            FROM transactions_fts
            JOIN transactions_fts_map map ON transactions_fts.rowid = map.fts_rowid
            JOIN pocketsmith_transactions t ON map.transaction_id = t.id
            WHERE transactions_fts MATCH ?1
              AND map.source = 'pocketsmith'",
        );
        ...
        if let Ok(mut stmt) = conn.prepare(&sql) {
            if let Ok(rows) = stmt.query_map(param_refs.as_slice(), |row| {
                ...
                metadata.insert("source".into(), serde_json::Value::String("pocketsmith".into()));
                let amt = amount.unwrap_or(0.0);
                let cat = category.unwrap_or_else(|| "Uncategorized".into());
                Ok(SearchResult {
                    result_type: "transaction".into(),
