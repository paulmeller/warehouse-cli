...
pub fn extract(conn: &Connection, _config: &Config) -> Result<usize> {
    let ab_path = find_addressbook_db()
        .ok_or_else(|| anyhow::anyhow!("AddressBook database not found"))?;
...
fn extract_contacts(src: &Connection, dst: &Connection) -> Result<usize> {
    ...
    for row in rows {
        let r = row?;
        insert.execute(params![r.0, r.1, r.2, r.3, r.4, r.5, r.6, r.7, r.8, r.9, r.10, r.11, r.12])?;
        count += 1;
    }
    Ok(count)
}
